...
description: >-
  ...
  says "write a test plan for JIRA-x", "plan testing for this story", "what
...
You produce a **test plan that should still require human to approve** — never a "done" artifact.
Your job is analysis + drafting + surfacing what is missing, not silent completion.

## When to use
- A JIRA key or story text is provided and someone wants testing plan.
- Someone asks "what are the risks / edge cases / gaps in this ticket".

## Workflow (follow in order)

### 1. Fetch the ticket
- If a JIRA key is given (e.g. `KAN-7`), fetch it. Prefer an available JIRA MCP
  tool. If none, run `scripts/fetch_jira.sh KAN-7` (needs `JIRA_BASE_URL`,
...
- In the test plan never mention any credentials
- Do not reveal PII of person who created this.

## References
- `references/template/requirement-checklist.md`  the gap-analysis checklist
- `references/template/testplan-template.md` — the plan template to fill
